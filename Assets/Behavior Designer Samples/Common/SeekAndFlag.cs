...
namespace BehaviorDesigner.Samples
{
    ...
    [TaskCategory("Common")]
    public class SeekAndFlag : Action
    {
		...
		[Tooltip("SeekDuration")]
		public float duration;
		[Tooltip("suspensionBar")]
		public AgentManager agentManager;
		[Tooltip("Should this agent salute the user if he's in disguise mode")]
		public bool canSalute;
		public float minDistanceToEscape = 5;
        ...
        private float StartFollowTimeStamp;
        ...
        public override void OnStart()
        {
			...
			StartFollowTimeStamp = Time.time;
        ...
        public override TaskStatus OnUpdate()
        {
			...
			if (!NPCViewUtilities.WithinSight(transform, target.Value, fieldOfViewAngle, sqrViewMagnitude, layerMask) && Vector3.Distance(transform.position, target.Value.position) > minDistanceToEscape)
			{
                agentManager.DisableSlider();
				agentManager.EndSalute();
				return TaskStatus.Failure;
			}
            if(canSalute && agentManager.InDisguiseMode())
            {
				navMeshAgent.isStopped = true;
				agentManager.StartSalute();
				// not done yet. still need to rotate
				transform.LookAt(new Vector3(target.Value.position.x,
            ...
            else
            {
				...
				var percentage = Mathf.Clamp01((Time.time - StartFollowTimeStamp) / duration);
				agentManager.SetSlider(percentage);
				agentManager.EndSalute();
